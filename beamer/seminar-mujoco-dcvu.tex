...
\end{frame}
	...
	\begin{frame}
		\frametitle{Comparison}
			...
			\scalebox{0.63}{\input{tikz/compare.tex}}
	...
	\begin{frame}
		...
		\scalebox{0.8}{\input{tikz/mjc-resource.tex}}
	\end{frame}
...
pip install mujoco
	...
	\begin{frame}
		...
		\begin{figure}
			\scalebox{0.6}{\input{tikz/mujoco-approach.tex}}
		\end{figure}
...
for i in range(7):
	data.joint(f"joint{i+1}").qpos = joint_demand[i] - angles_offset[i]
			...
			\begin{figure}
				\scalebox{0.7}{\input{tikz/7dof.tex}}
			\end{figure}
  ...
  return result.x, result.cost
		...
		\subsection{6-DOF parallel mechanisms}
			...
			\begin{frame}[fragile]
				...
				\begin{figure}
					\scalebox{0.7}{\input{tikz/parallel-mechanisms.tex}}
				\end{figure}
...
x = normalize_pos_workingspace(round_to_step(self.joystick.get_axis(4)))			
				\end{minted}
				\scalebox{0.6}{\input{tikz/joystick.tex}}
			\end{frame}
